...
SELECT DISTINCT REGEXP_INSTR (name, '[[:alpha:]]+', 1, 1, 0) AS first_name FROM athlete LIMIT 5;
...
SELECT REGEXP_INSTR(EMAIL, '[^@]+', 1, 1) AS ""id""
, REGEXP_INSTR(EMAIL, '[^@]+', 1, 2) AS ""domain""
FROM V_TEST;

DROP TABLE IF EXISTS first_name_store;
CREATE TABLE first_name_store ( first_name VARCHAR ) AS SELECT DISTINCT REGEXP_INSTR (name, '[[:alpha:]]+', 1, 1) AS first_name FROM athlete LIMIT 10;
SELECT * FROM first_name_store;

DROP TABLE IF EXISTS first_name_store;
CREATE TABLE first_name_store ( first_name VARCHAR );
INSERT INTO first_name_store SELECT DISTINCT REGEXP_INSTR (name, '[[:alpha:]]+', 1, 1) AS first_name from athlete LIMIT 10;
SELECT * FROM first_name_store;
