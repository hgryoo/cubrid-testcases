...
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'^[a-zA-Z0-9].@[0-3]$','i') order by 1 desc,2,3,4,5,6,7;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'(txt)?%$','c') order by 2 desc,3,4,1,6,5,7;
select id, d, t, f||'' f, s, m, b from part01 where not regexp_like(d, '2010$') order by 7,6,5,4,3 desc,2,1;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(cast(id as varchar(10)),'^[^20].(02)$') order by 1,2,3,4,5,6,7;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'^[b]|[a]+$') order by 1,2,3,4,5,6;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'[[.tab.]]') and not regexp_like(s,'[[.tilde.]]');
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(cast(f as varchar(10)),'^[0-9]+.[0-9]+$') or regexp_like(cast(f as varchar(10)),'^[0-9]+$') order by 1,2,3,4,5,6,7;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'^[[:alnum:]]+$') or not regexp_like(s,'[[:alpha:]]') order by 1,2,3,4,5,6,7;
select id, d, t, f||'' f, s, m, b from part01 where regexp_like(s,'\b[a-z]{5,6}\b') order by 2,1,3,4,5,7,6;
